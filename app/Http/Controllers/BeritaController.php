...
class BeritaController extends Controller
{
    ...
    public function show()
    {
        ...
        $beritaPopulerMingguan = Cache::remember("beritaPopulerMingguan", now()->addHours(1), function () use ($startOfRange, $endOfRange) {
            ...
            $mingguan = Berita::whereBetween('created_at', [$startOfRange, $endOfRange])
                ->where('status', 'show')
                ->orderBy('hit', 'desc')
                ->take(6)
                ->get();
            ...
            if ($mingguan->count() < 6) {
                ...
                $mingguan = $mingguan->concat($tambahan);
            }
    ...
    public function getDataTables(Request $request)
    {
        ...
        return DataTables::of($berita)
            ->addColumn('aksi', function ($row) {
                ...
                <button class="cursor-pointer editBtn bg-blue-500 text-white px-2 py-1 rounded text-sm" data-id="' . e($row->id_berita) . '">Edit</button>
                <button class="cursor-pointer deleteBtn bg-red-500 text-white px-2 py-1 rounded text-sm ml-2" data-id="' . e($row->id_berita) . '">Hapus</button>
                </div>
            ';
            })
