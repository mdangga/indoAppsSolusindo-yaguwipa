                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            if (type === 'display') {
                                ...
                                // Debug: Console log the data to see what we're receiving
                                console.log('Data received:', data);

                                // Cek apakah data berisi HTML video atau img
                                if (typeof data === 'string') {
                                    // Cek untuk video - improved regex
                                ...
                                // Jika tidak ada match dari HTML, coba langsung sebagai URL
                                if (!mediaUrl && typeof data === 'string') {
                                    // Cek apakah data adalah URL langsung
                                    if (data.match(/\.(mp4|webm|ogg)$/i)) {
                                        mediaUrl = data;
                                        isVideo = true;
                                    } else if (data.match(/\.(jpg|jpeg|png|webp)$/i)) {
                                        mediaUrl = data;
                                        isVideo = false;
                                    } else if (data.startsWith('http') || data.startsWith('/')) {
                                        // Assume it's a video if it's a URL without clear extension
                                        mediaUrl = data;
                                        isVideo = true;
                                    }
                                }
                        ...
                        <div class="flex justify-center">
                            <video controls preload="metadata" class="w-32 h-20 object-cover border border-gray-200 shadow-sm rounded"
                                   style="max-width: 128px; max-height: 80px;"
                                   onloadstart="console.log('Video loading started')"
                                   oncanplay="console.log('Video can play')"
                                   onerror="console.log('Video error:', this.error); this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <source src="${mediaUrl}" type="video/mp4">
                                <source src="${mediaUrl.replace(/\.[^.]+$/, '.webm')}" type="video/webm">
                                <source src="${mediaUrl.replace(/\.[^.]+$/, '.ogg')}" type="video/ogg">
                                Your browser does not support the video tag.
                            </video>
                            <div class="w-32 h-20 bg-gray-100 border border-gray-200 flex items-center justify-center rounded" style="display: none;">
                                <i class="fas fa-video text-gray-400"></i>
                                <span class="text-xs text-gray-500 ml-1">Video Error</span>
                            </div>
                        </div>
                    `;
                                    } else {
                        ...
                        <div class="flex justify-center">
                            <img src="${mediaUrl}" alt="Thumbnail"
                                ...
                                onerror="console.log('Image error'); this.onerror=null; this.src='/img/no-image.png'; this.classList.add('opacity-50');">
                        </div>
                    `;
                                    }
                                } else {
