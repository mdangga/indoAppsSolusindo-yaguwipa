    ...
    <script>
        $(document).ready(function() {
            let table = $('#beritaTable').DataTable({
                ...
                dom: '<"flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6"<"flex items-center gap-2"l><"flex items-center gap-2"f>B>rt<"flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6"ip>',
                buttons: [{
                        extend: 'excel',
                        text: '<i class="fas fa-file-excel mr-2"></i>Excel',
                        className: 'inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition-all duration-200 shadow-sm hover:shadow-md',
                        exportOptions: {
                            columns: [0, 1, 3] // Specify which columns to export (0-based index)
                        }
                    },
                    {
                        extend: 'pdf',
                        text: '<i class="fas fa-file-pdf mr-2"></i>PDF',
                        className: 'inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg transition-all duration-200 shadow-sm hover:shadow-md',
                        exportOptions: {
                            columns: [0, 1, 3] // Specify which columns to export
                        }
                    },
                    {
                        extend: 'print',
                        text: '<i class="fas fa-print mr-2"></i>Print',
                        className: 'inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-medium rounded-lg transition-all duration-200 shadow-sm hover:shadow-md',
                        exportOptions: {
                            columns: [0, 1, 3] // Specify which columns to export
                        }
                    }
            ...
            // Add the buttons container to your DOM
            table.buttons().container()
                .appendTo($('.flex.items-center.gap-2:first'));
