...
<body class="bg-gray-50">
    ...
    <main class="p-6 sm:ml-64 pt-24 transition-all ">
        ...
        <div class="w-full bg-white p-6 rounded shadow">
            <h1 class="text-2xl font-bold mb-4">{{ isset($menus) ? 'Edit Menu' : 'Tambah Menu' }}</h1>
            ...
            <form id="formMenus" action="{{ $isEdit ? route('menus.update', $menu->id_menus) : route('menus.store') }}"
                method="POST">
                @csrf
                @if ($isEdit)
                    @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-6 mt-4">
                    <div>
                        <label for="parent_menu" class="block mb-1 text-sm font-medium text-gray-900">Parent
                            Menu</label>
                        <select id="parent_menu" name="parent_menu"
                            class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5">
                            <option value="">Tidak ada</option>
                            @foreach ($menus as $menu)
                                <option value="{{ $menu->id_menus }}"
                                    {{ old('parent_menu') == $menu->id_menus ? 'selected' : '' }}>
                                    {{ $menu->title }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-4">
                    ...
                    <input id="url" type="text" name="url"
                        class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5"
                        value="{{ old('url', $isEdit ? $menu->url : '') }}" required placeholder="Masukkan url..." />
                    @error('url')
                        <small class="text-red-600">{{ $message }}</small>
                    @enderror
                </div>
                <div class="mt-4">
                    ...
                    <select id="status" name="status"
                        class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5">
                        <option value="show" {{ old('status', $berita->status ?? '') == 'show' ? 'selected' : '' }}>
                            Show</option>
                        <option value="hide" {{ old('status', $berita->status ?? '') == 'hide' ? 'selected' : '' }}>
                            Hide</option>
                    </select>
                    @error('status')
                        <small class="text-red-600">{{ $message }}</small>
