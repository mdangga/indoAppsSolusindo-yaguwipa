        ...
        <div class="px-4 sm:px-6 lg:px-12 py-16">
            <div class="max-w-7xl mx-auto">
                <x-header-page title="PUBLIKASI"
                    description="Koleksi dokumen publikasi resmi yayasan, termasuk laporan, artikel, dan materi informatif lainnya." />
                <!-- Filter Section -->
                ...
                {{-- rekomendasi --}}
                @if (isset($mostDownloaded))
                    <div id="mostDownloadedSection" class="mb-12">
                        <div class="flex items-center gap-4 mb-4">
                            <h1 class="text-3xl font-semibold text-gray-800 whitespace-nowrap">Most Downloaded</h1>
                            <div class="flex-grow border-b-4 border-gray-500"></div>
                        </div>

                        <div id="mostDownloadedGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
...
<div class="absolute top-3 right-3">
    <span class="bg-blue-500 text-white px-2 py-1 rounded-full text-xs font-medium">
        ...
        @endif
        {{-- lainnya --}}
        <div id="otherPublicationsSection">
            <div class="flex items-center gap-4 mb-4">
                <h1 class="text-2xl font-semibold text-gray-700 whitespace-nowrap">Lainnya</h1>
                <div class="flex-grow border-b-4 border-gray-500"></div>
            </div>
    ...
    @empty
        ...
        <div id="noResults" class="hidden col-span-full text-center py-12">
            ...
            <h3 class="text-xl font-medium text-gray-600">Publikasi tidak ditemukan</h3>
            <p class="text-gray-500 mt-2">Tidak ada hasil yang sesuai dengan pencarian Anda</p>
        </div>
        <!-- Modal -->
        <div id="publicationModal"
            ...
            <div
                ...
                <div class="mt-auto pt-4 text-right">
                    <a id="modalDownloadLink" href="#" target="" onclick="downloadFile()"
    ...
    <script>
        ...
        const mostDownloadedSection = document.getElementById('mostDownloadedSection');
        const otherPublicationsSection = document.getElementById('otherPublicationsSection');

        // Ambil semua card dari bagian "Lainnya"
        const cards = Array.from(document.querySelectorAll('#otherPublicationsSection .publication-card'));

        // Fungsi untuk mengecek apakah filter aktif
        function isFilterActive() {
            return searchInput.value !== '' ||
                categoryFilter.value !== '' ||
                sortFilter.value !== 'newest'; // 'newest' adalah nilai default
        }

        // Fungsi untuk mengatur tampilan most downloaded section
        function updateMostDownloadedVisibility() {
            if (mostDownloadedSection) {
                if (isFilterActive()) {
                    mostDownloadedSection.classList.add('hidden');
                } else {
                    mostDownloadedSection.classList.remove('hidden');
                }
            }
        }
        ...
        function filterAndSortPublications() {
            ...
            // Tampilkan card yang lolos filter dengan urutan baru
            if (visibleCards.length > 0) {
                ...
                otherPublicationsSection.classList.remove('hidden');
            } else {
                noResults.classList.remove('hidden');
                otherPublicationsSection.classList.add('hidden');
            }
        }
